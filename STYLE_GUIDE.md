...
7. **Loop** τέλος → αρχή: το τελευταίο frame = το πρώτο (ίδιο caption, ίδια εικόνα) → `LOOP: true` στο `render.js` (wipe 0,3s → frame 0). Το lint το ελέγχει.
...
- **«Πώς φτιάχνεται;»** — η διαδικασία βήμα-βήμα (laser κ.λπ.) με τον Στράτο, CTA «Στείλε μήνυμα».
