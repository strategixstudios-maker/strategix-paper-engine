...
- `node render.js vo <mp3>` → φράσεις + παύσεις → `node render.js vo <mp3> --gap 0.3 --out vo/<ep>_vo.mp3 --at 0.2`: κάθε παύση > 0,3s γίνεται 0,3s → `vo/<ep>_vo.mp3` (μπαίνει στο git) + χρονισμοί φράσεων σε χρόνο video → σχόλιο στην κορυφή του επεισοδίου. Μετά: `node ep.js vo` (ίδιος πίνακας από το `VO_FILE`).
- **Καμία «δραματική» παύση πριν από punchline** (pm02: ο θεατής χάνει το ενδιαφέρον στη σιωπή): το punchline λέγεται κολλητά → `--keep N:0.03` (N = η φράση του punchline). Στο κείμενο του VO **όχι «...» πριν από punchline** (το `eleven_v3` τραβάει τη λέξη)· αν βγει τραβηγμένη, atempo ~1.15 μόνο σε εκείνο το κομμάτι πριν το σφίξιμο. `--keep N:0.45` μόνο σε αλλαγή σκηνής (π.χ. σκίσιμο οθόνης).
