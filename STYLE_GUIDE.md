...
- **Αλλαγή μίας ατάκας** σε έτοιμο VO (pf03 v3, pf04 v2): νέο take **μόνο** εκείνης της φράσης → `node render.js vo vo/<ep>_vo.mp3 --splice <new.mp3> --from N [--to M] [--tempo 1.06] --out vo/<ep>_vo.mp3` (οι φράσεις N..M αντικαθίστανται, ίδια ένταση ομιλίας, οι υπόλοιποι χρονισμοί μένουν ίδιοι).
...
- **«Πώς φτιάχνεται;»** — η διαδικασία βήμα-βήμα (laser κ.λπ.) με τον Στράτο, CTA «Στείλε μήνυμα». Κάθε βήμα δείχνει το **πραγματικό εργαλείο** να κάνει τη δουλειά (πιστόλι πιεστικού νερού, σπάτουλα, laser) και το αποτέλεσμα να εμφανίζεται εκεί που περνάει. Όχι σκέτο χέρι που δείχνει. Κάθε βήμα λέει **τι κάνουμε + γιατί** (αιτία → αποτέλεσμα, με το εργαλείο ονομασμένο: «ανάβει το UV φως… όπου περνάει σκληραίνει», όχι σκέτο «Φως.»). Ό,τι δεν φαίνεται με το μάτι (φως, χημεία, θερμότητα) → ετικέτα `stamp()` πάνω στην περιοχή που αλλάζει + αλλαγή χρώματος. Τεστ: όποιος δεν ξέρει τη διαδικασία καταλαβαίνει το βήμα χωρίς ήχο (pf03 v3: το βήμα «Φως» δεν το κατάλαβαν θεατές).
