...
- [pf03] Πρώτη μεταξοτυπία → όλα τα screenprint props μέσα στο επεισόδιο (matBG cutting mat, teeFlat, frame/screen states mesh|emulsion|film|stencil, stencilInner + inkBead + press (τελάρο στο tee, σηκώνεται μαζί με ό,τι έχει πάνω), grip (χέρι στη λαβή), squeegee (≠ `squeegee` του pm01 = ράκλα βινυλίου → άλλο όνομα στο props/), filmPositive, platen heat-press, spray). Στη 2η μεταξοτυπία → `props/screenprint.js` (+ `node api.js --check`).
